...
NOTE: In order to use a different SQL Server instance then local SQL Server Express, make sure the connection strings are updated in all places.

## Running the project

 1. Start the projects: Adapter, Sales and Shipping (right-click on the project, select the `Debug > Start new instance` option).
 1. Open ServicePulse and select the Endpoints Overview view. `Samples.ServiceControl.SqlServerTransportAdapter.Shipping` endpoint should be visible in the Active Endpoints tab as it has the Heartbeats plugin installed
